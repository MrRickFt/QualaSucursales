...
namespace BackEndSucursales.Persistence.Repositories
{
	public class SucursalRepository : ISucursalRepository
	{
		...
		public async Task<Sucursal> GetSucursalAsync(int codigoSucursal)
		{
			try
			{
				var sucursal = await _context.Sucursal_Quala.Where(x => x.Codigo == codigoSucursal).Include(s => s.Moneda).FirstOrDefaultAsync();
				return sucursal;
			} catch (Exception ex)
			{
				return null;
		...
		{
			var listaSucursal = await _context.Sucursal_Quala.Include(s => s.Moneda).ToListAsync();
		...
		{
			var entidadOriginal = await _context.Sucursal_Quala.FindAsync(codigoSucursal);
		...
		{
			var sucursal = await _context.Sucursal_Quala.FindAsync(codigoSucursal);
			if (sucursal != null)
			{
				_context.Sucursal_Quala.Remove(sucursal);
				await _context.SaveChangesAsync();
				return sucursal;
			}
			else
